...
step_size = 0.5
bezier_option = 'cubic'
# good one to play with (to guarantee clipping, set to False)
...
# either 'path' or 'group' (case-sensitive); 'group' is faster but less entertaining
NO_ANIM_UPDATE = 'group'
NO_ANIM_UPDATE_RATE = 30
...
def cubic_bezier(P0, P1, P2, P3):
    ...
    return np.dot(t_mat, np.dot(M, P_mat))
...
def angle_dist(P0, P1):
    ...
    P0_x, P0_y = P0
    P1_x, P1_y = P1
    distance = sqrt((P0_x - P1_x) * (P0_x - P1_x) +
                    (P0_y - P1_y) * (P0_y - P1_y))
...
def parse_path(Mx, My, clz):
    ...
    def _rel_draw(dx, dy):
        ...
        if mode == 'curve' and len(ctrl_pts_tur) < num_req_pts:
            ...
            turtle_traverse(
                np.array([ctrl_pts_tur[-1], abs_pt_tur]), setpos=False)
            del ctrl_pts_svg[:]
            del ctrl_pts_tur[:]
            ctrl_pts_svg.append(abs_pt_svg)
            ctrl_pts_tur.append(abs_pt_tur)
    ...
    for dx, dy in zip(*[iter(clz)] * 2):
        if dx[0] == 'c':
            ...
            curr_pt_svg = (
                ctrl_pts_svg[-1][0] + float(dx[1:]), ctrl_pts_svg[-1][1] + float(dy))
        ...
        _rel_draw(dx, dy)
        if close:
            _rel_draw(prev_move_svg[0] - ctrl_pts_svg[-1]
                      [0], prev_move_svg[1] - ctrl_pts_svg[-1][1])
            close = False
...
if __name__ == '__main__':
    ...
    vb_min_x, vb_min_y, vb_width, vb_height = svgroot.attrib.get(
        'viewBox', None).split()
    vb_min_x, vb_min_y, vb_width, vb_height = [
        float(d) for d in (vb_min_x, vb_min_y, vb_width, vb_height)]
    ...
    if canvas_aspect_ratio == view_box_aspect_ratio:
        ...
        y_scale = float(canvas_height) / vb_height
        x_scale = y_scale
    # translation to apply to all coords
    x_shift, y_shift = -float(canvas_width) / 2, -float(canvas_height) / 2
    ...
    if draw_boundary:
        turtle_traverse(
            [turtle_00, turtle_w0, turtle_wh, turtle_0h, turtle_00])
    ...
    def handle_path(path):
        ...
        Mx, My, clz = float(d[0][1:]), float(d[1]), d[2:]
        if fill_shapes:
            turtle_begin_fill()
        ...
        if save_output:
            ...
            if python_ver >= 3:
                ...
                with open(svgfile) as svg_input, open(outfile, 'wb') as png_output:
                    cairosvg.svg2png(
                        bytestring=svg_input.read(), write_to=png_output)
                import shutil
